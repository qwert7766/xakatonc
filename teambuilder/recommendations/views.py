...
def show_recommendation(request, profile_id):
    ...
    for rec in all_recommendations:
        if rec['employee'].id not in processed_ids:
            ...
            rating_logs = RecommendationLog.objects.filter(
                manager=request.user,
                employee=employee,
                status='rejected'
            )
            
            # Ищем оценку в комментарии (формат: "Оценка: X/5. Причина: ...")
            rating = None
            for log in rating_logs:
                match = re.search(r'Оценка:\s*(\d+)/5', log.comment or '')
                if match:
                    rating = int(match.group(1))
                    break
            
            top_candidate['rating'] = rating
            break
